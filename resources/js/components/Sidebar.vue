<template>
    <nav
        class="bg-[#4047F3] w-48 min-h-screen pt-8 text-white space-y-22 hidden md:block"
    >
        <div class="flex flex-row items-center gap-x-3 justify-center">
            <img
                src="http://localhost:8000/image/parikesit.png"
                alt="Foto Profil"
                class="w-8 h-8"
            />
        ...
        <ul class="pl-3">
            <li
                ...
                :key="item.to"
                class="relative py-3 text-[13px]"
                :class="{ 'with-right-bump': isActiveRoute(item.to) }"
            >
                <router-link
                    :to="item.to"
                    :class="{
                        'pl-4 text-[#4047F3] flex gap-x-2': isActiveRoute(
                            item.to
                        ),
                        'pl-4 hover:text-blue-200 transition duration-200 flex gap-x-2':
